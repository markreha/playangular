...
3. Deploy from a Build:
	...
	5. Run the 'npm run build --prod' command (if you make any changes run a 'git pull [GIT REPO URL]' command and run the 'npm run build --prod' command again).
	6. Update the package.json file (you can use the build in Code Editor in the Cloud Shell) with the following changes to the scripts, replace APP_NAME accordingly:
		* Update: "start": "serve -s dist/[APP NAME]",
...
4. Deploy from a GIT CI/CD Build Pipeline:
	...
	5. Create a Google Cloud Build file named cloudbuild.yaml and add this to the root of the project in GitHub.
